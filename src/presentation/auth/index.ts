 ...
 */
export const authPlugin = (authController: AuthController = new AuthController()) => {

    return new Elysia({ prefix: "/auth", name: "Auth" })
        .use(tokenPlugin)
        .post("/register",  async ({ status, body }) => {        
            const usuario = await authController.doRegister(body)
            return status(201, { message: `Usuario con ${usuario.correo} creado` })
        }, {
            body: AuthModel.signUpBody
        })
        .put("/", async ({ status, body, tokenPlugin }) => {
...
export const authRoutes = authPlugin()